"""Benchmark sensitivity to PINN data/PDE loss weights."""
...
WEIGHT_SETTINGS = [
    ...
    {"setting": "high_pde", "data_weight": 8.0, "pde_weight": 0.36},
    {"setting": "low_data", "data_weight": 4.0, "pde_weight": 0.12},
    {"setting": "high_data", "data_weight": 12.0, "pde_weight": 0.12},
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run data/PDE loss weight sensitivity benchmark.")
...
def main() -> None:
    ...
    for target_gene in args.target_gene:
        rows.extend(run_target(args, target_gene))
    summary = OUTPUT_ROOT / args.sample / "loss_weight_sensitivity_summary.csv"
    write_csv(rows, summary)
    print(f"Wrote loss weight sensitivity summary to {summary}", flush=True)


def run_target(args: argparse.Namespace, target_gene: str) -> list[dict[str, object]]:
    sample_dir = PROCESSED_ROOT / args.sample
    preflight_dir = PREFLIGHT_ROOT / args.sample / f"{target_gene}_CNS_Myelin" / args.histology_prior
    output_dir = OUTPUT_ROOT / args.sample / f"{target_gene}_CNS_Myelin"
    ...
    for weights in WEIGHT_SETTINGS:
        ...
        if args.skip_existing and masked_path.exists() and gauss_path.exists():
            ...
            profile = replace(
                base_profile,
                name=f"{args.profile}_{setting}",
                data_weight=float(weights["data_weight"]),
                pde_weight=float(weights["pde_weight"]),
            )
            print(
                f"Running loss-weight sensitivity: {target_gene} {setting} "
    ...
    write_csv(rows, output_dir / "loss_weight_sensitivity_metrics.csv")
    return rows
