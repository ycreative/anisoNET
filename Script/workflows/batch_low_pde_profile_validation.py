"""Validate low-PDE loss weighting across all GSE193107 brain sections."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run default vs low-PDE profile validation.")
...
def main() -> None:
    ...
    summary_path = OUTPUT_ROOT / "low_pde_profile_validation_metrics.csv"
    ...
    manifest = {
        ...
        "source_percentile": args.source_percentile,
        "output_percentile": args.output_percentile,
        "summary_csv": str(summary_path),
    }
    (OUTPUT_ROOT / "low_pde_profile_validation_manifest.json").write_text(
...
def run_sample_target(args: argparse.Namespace, sample: str, target_gene: str) -> list[dict[str, object]]:
    ...
    for weights in WEIGHT_SETTINGS:
        ...
        if args.skip_existing and masked_path.exists() and gauss_path.exists():
            ...
            profile = replace(
                base_profile,
                name=f"{args.profile}_{setting}_{sample}_{target_gene}",
                data_weight=float(weights["data_weight"]),
                pde_weight=float(weights["pde_weight"]),
            )
            print(
                f"Running {sample} {target_gene} {setting}: "
    ...
    write_csv(rows, output_dir / "low_pde_profile_validation_metrics.csv")
    return rows
